...
public class DataWriter implements DataConstants {
    ...
    public DataWriter(final String filename, final boolean mode) throws IOException {
	this.dataMode = mode;
    ...
    public DataWriter(final File filename, final boolean mode) throws IOException {
	this.dataMode = mode;
	if (mode == DataConstants.DATA_MODE_TEXT) {
	    this.bw = new BufferedWriter(new FileWriter(filename));
	} else {
